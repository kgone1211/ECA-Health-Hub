...
export function getWhopUserFromUrl(): string | null {
  if (typeof window === 'undefined') return null;
  
  const urlParams = new URLSearchParams(window.location.search);
  return urlParams.get('userId') || urlParams.get('whop_user_id') || urlParams.get('user_id');
}

export function getWhopCompanyFromUrl(): string | null {
  if (typeof window === 'undefined') return null;
